...
from config import (
    ...
    PERMANENT_LOAD_TEST, DEMAND_REPEAT_PER_SEASON, DIAG_PRINTS,
)
...
def _stitch_full_year_from_seasons(
    ...
    season_by_month = _SEASON_BY_MONTH_SOUTH
...
def simulate_household_efficient(
    ...
    try:
        ...
        def _to_5min_litres_only(season_Lpm: pd.Series) -> pd.Series:
            """Convert 1-min L/min series to 5-min litre totals (no energy calc here)."""
        ...
        for season, ser in seasonal_Lpm.items():
            seasonal_L5[season] = _to_5min_litres_only(ser)

        city_is_north = False  # All cities in this version are Southern Hemisphere
        ...
        # Calculate element resistance (R = V^2 / P)
        ...
        # demand_kwh_sum is accumulated inside the loop using the active setpoint
        # (65 °C when PV is available, 55 °C when grid-only) so the reference energy
        # always matches the actual thermostat target for that timestep.
        demand_kwh_sum = 0.0

        # draw_kwh is filled step-by-step in the loop below (same active-setpoint logic).
        draw_kwh_vals = np.zeros(len(idx), float)
        ...
        for i, ts in enumerate(idx):
            ...
            # near-zero nighttime/cloudy values reliably for all cities and seasons.
            pv_allowed = pv_available
            ...
            # Demand energy for this step: use the active setpoint so the reference
            # is 65°C during PV-mode and 55°C during grid-mode — never a fixed 65°C.
            step_demand_kwh = (
                v_draw * 4184.0
                * max(0.0, active_sp - current_inlet_temp)
                / 3_600_000.0
            )
            demand_kwh_sum   += step_demand_kwh
            draw_kwh_vals[i]  = step_demand_kwh

            if PERMANENT_LOAD_TEST:
                heater_on = True
            else:
                if not heater_on:
                    if tank.top_temp <= on_thr:
                        heater_on = True
            ...
            # Baseline Tank Control (Grid Only - fixed at grid setpoint)
            ...
            if PERMANENT_LOAD_TEST:
                ...
                if not heater_on_baseline:
                    if tank_baseline.top_temp <= base_on_thr:
                        heater_on_baseline = True
            ...
            if T_out_before > current_inlet_temp:
                if T_out_before >= T_use:
                    ...
                    # Tank is colder than 40°C, user gets a lower effective volume of "hot" water
        ...
        # Build draw_kwh timeseries from per-step values accumulated in the loop
        draw_kwh = pd.Series(draw_kwh_vals, index=idx, dtype=float)
        ...
        if DIAG_PRINTS:
            ...
            dt_h_diag = float(TANK_PARAMS['dt_s']) / 3600.0
            sim_hours_total = sim_steps_total * dt_h_diag
            sim_range_min, sim_range_max = idx[0], idx[-1]
            pv_kwh_possible_total = float((pv_kw * dt_h_diag).sum())
            avg_mains = float(np.mean(mains_temp_vals))
            dT_avg = max(1e-6, setpoint_grid - avg_mains)
            total_litres_est = float(demand_kwh_sum * 3_600_000.0 / (4184.0 * dT_avg))
            ...
            print(f"[{household_id}] Avg NREL mains temp: {avg_mains:.1f}°C")
            print(f"[{household_id}] Heater ON steps: {on_steps_total}/{sim_steps_total} → hours_on={on_steps_total * dt_h_diag:.1f}")
